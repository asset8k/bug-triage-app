"""
Evaluate fine-tuned Qwen 2.5 7B (GGUF) on the holdout test set.
Uses dataset/test.jsonl only (from dataset/2_convert_to_jsonl.py); optional --batch-size to subsample.
Writes results/qwen_finetuned_metrics.csv for the LLM Results Table.
"""
...
METRICS_CSV = RESULTS_DIR / "qwen_finetuned_metrics.csv"
PREDICTIONS_CSV = RESULTS_DIR / "qwen_finetuned_predictions.csv"
...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate fine-tuned Qwen 2.5 7B GGUF model.")
...
def main() -> None:
    ...
    try:
        from src.engines.qwen_finetuned import QwenFinetunedEngine
    except Exception as e:
        print(f"Failed to import QwenFinetunedEngine: {e}", file=sys.stderr)
        sys.exit(1)
    ...
    print(
        f"\nQwen 2.5 7B (Fine-tuned) — Accuracy: {m['accuracy']:.4f} | Macro F1: {m['macro_f1']:.4f} | Duration: {duration_sec:.1f}s"
    )
