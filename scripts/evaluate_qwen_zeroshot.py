"""
Evaluate zero-shot severity classification using Qwen 2.5 7B via local Ollama API.
...
PREDICTIONS_CSV = RESULTS_DIR / "qwen_zeroshot_predictions.csv"
METRICS_CSV = RESULTS_DIR / "qwen_zeroshot_metrics.csv"
...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate Qwen 2.5 7b zero-shot severity classification.")
...
def main() -> None:
    ...
    # Call Qwen via Ollama for each row
    ...
    print("\nQwen 2.5 7B (zero-shot) — Accuracy: {:.4f} | Macro F1: {:.4f} | Precision: {:.4f} | Recall: {:.4f} | Duration: {:.1f}s".format(
