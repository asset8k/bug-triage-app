"""
Ollama inference: Qwen 2.5 7B via local Ollama API for severity classification.
...
# so zero-shot Qwen sees the same task description as the fine-tuned model.
INSTRUCTION = (
...
class QwenEngine(BaseEngine):
    """Severity prediction via local Ollama (Qwen 2.5 7B)."""

    def __init__(self, model_id: str = "qwen") -> None:
    ...
    def predict(self, text: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(
                "Cannot reach Ollama at localhost:11434. Start Ollama and run: ollama run qwen2.5:7b"
            )
        except requests.exceptions.Timeout:
            raise RuntimeError("Qwen request timed out. Try again or use a baseline model.")
